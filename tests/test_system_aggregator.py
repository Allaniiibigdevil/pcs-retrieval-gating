...
def test_system_aggregator_decision_thresholds() -> None:
    aggregator = SystemAggregator(retrieve_threshold=0.8, maybe_retrieve_threshold=0.55)
    ...
    assert by_system["memo_system"].decision == "retrieve"
    assert by_system["album_system"].decision == "maybe_retrieve"
    assert by_system["todo_system"].decision == "skip"
