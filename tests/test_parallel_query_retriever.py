...
@pytest.mark.asyncio
async def test_duplicate_documents_keep_best_channel_hit_and_receive_new_rank() -> None:
    ...
    assert [hit.doc_id for hit in result.keyword.hits] == ["shared", "keyword-only"]
    assert result.keyword.hits[0].bm25_score == 4.0
    assert [hit.bm25_rank for hit in result.keyword.hits] == [1, 2]
