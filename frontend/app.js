  ...
  systemList.innerHTML = data.decisions
    .map((decision) => {
      const evidence = (decision.evidence_docs || [])
        .map(
          (doc) => `<article class="doc-card">
            ...
            <div class="metrics">
              <span>Reranker ${formatNumber(doc.reranker_score)} ${formatRank(doc.reranker_rank)}</span>
              <span>BM25 ${formatNumber(doc.bm25_score)} ${formatRank(doc.bm25_rank)}</span>
              <span>Vector ${formatNumber(doc.vector_score)} ${formatRank(doc.vector_rank)}</span>
            </div>
      ...
      return `<article class="system-card ${decision.selected ? "selected" : ""}">
        ...
        <div class="confidence">best reranker score ${formatNumber(decision.reranker_score)}</div>
